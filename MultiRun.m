...
% Plot Parameters    
T_gif = 150; %150;  % Time step to finish on for gif
...
for i_case = 1:numel(summary)
    n = summary(i_case).inputs.n;
    n_e = summary(i_case).inputs.n_e;
    n_f = summary(i_case).inputs.n_f;
    n_z = summary(i_case).inputs.n_z;
    x = summary(i_case).results.x;
