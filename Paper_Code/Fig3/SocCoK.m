...
x=[zeros((1-conf)*n,1); ones(conf*n,1)]; %Starting strategy for everyone
t=0;
flag=0;
z=sum(x)/n; %Social coordination
y=zeros((1-conf)*n,1);
ts=T;
while  t<T
    t=t+1;
    old=x(1:(1-conf)*n); %Copies original x array for non-CM
    x=zeros(n,1);
    x(rand(n,1)<exp(beta.*(z(end)))./(exp(beta.*z(end))+exp(beta.*(1-z(end)))))=1; %SC only
%If rand number [0,1] < prob(alt), then agent plays alt
    x((1-conf)*n+1:n)=1; %Manually overrides the CM's strategy to Alt
    z=[z sum(x)/n]; %Adds new array entry for social coordination (Also=proportion of pop playing alt)
    if flag==0
            y=y+abs(old-x(1:(1-conf)*n));
    end
    if z(end)>.95 && flag==0 %If %Alt>0.95 then process ends
        flag=1;
        ts=t; %Counts the total number of rounds
    end
end
if T>400
    z=reducev(z,0:T,200);
...
if ts<T %Dividing y by the number of completed rounds, depending on whether it ended early or not
    y=(y-1)/ts;
else
    y=y/T; 
end
